...
const SCOPES = 'user-read-playback-state user-read-currently-playing user-modify-playback-state playlist-read-private playlist-read-collaborative user-library-read';
...
function startAuthFlow(onComplete) {
  if(cancelAuth)cancelAuth('A newer sign-in was started.');
  const {verifier,challenge}=generatePkcePair(),state=base64url(crypto.randomBytes(24));
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    if(!code||url.searchParams.has('error')){res.end('Spotify connection cancelled.');finish({success:false,error:'Spotify authorization was cancelled.'});return;}
    if(settled){res.end('Sign-in already completed.');return;}
    try{const tokens=await exchangeCodeForTokens(code,verifier);if(settled){res.end('Sign-in expired. Please retry.');return;}saveTokens(tokens);res.end('Spotify connected. You can return to Workday.');finish({success:true});}
    catch(e){res.end('Spotify connection failed. Please retry in Workday.');finish({success:false,error:e.message});}
  });
  function finish(result){if(settled)return;settled=true;clearTimeout(timer);server.close();cancelAuth=null;onComplete(result);}
  cancelAuth=error=>finish({success:false,error});
  timer=setTimeout(()=>finish({success:false,error:'Sign-in timed out. Please retry.'}),180000);
  server.on('error',e=>finish({success:false,error:e.message}));
  server.listen(8888,'127.0.0.1',()=>{
    const url='https://accounts.spotify.com/authorize?'+new URLSearchParams({client_id:CLIENT_ID,response_type:'code',redirect_uri:REDIRECT_URI,code_challenge_method:'S256',code_challenge:challenge,scope:SCOPES,state,show_dialog:'true'});
    shell.openExternal(url).catch(e=>finish({success:false,error:e.message}));
  });
}
...
async function getPlayerState() {
  ...
  try {
    ...
    return {
      ...
      trackId: data.item && data.currently_playing_type==='track' ? (data.item.id || '') : '',
...
async function setShuffleMode(enabled) {
  if (typeof enabled !== 'boolean') return false;
  const accessToken = await getValidAccessToken();
  if (!accessToken) return false;
  try {
    const resp = await fetch('https://api.spotify.com/v1/me/player/shuffle?state='+enabled, {
...
module.exports = {
  startAuthFlow, isConnected, disconnect, getNextTrack, getValidAccessToken,
  webPlayPause, webNext, webPrevious,
  getPlayerState, getDevices, switchDevice, setVolume, seek, setRepeatMode, setShuffleMode
};
